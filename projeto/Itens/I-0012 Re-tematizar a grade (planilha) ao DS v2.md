...
A grade já vive no `#grid-scroll` do shell DS v2 (I-0010 resolvido). **Decisão [[D-0008 Grade isolada do tema e drawers theme-aware (dark)]]**: a grade é o **espelho do Excel e fica SEMPRE clara** (não se re-tematiza ao dark) — corrigido o vazamento do dark (texto das células ficava branco) em `.proj-main #grid-scroll:not(.dash):not(.solic)` + `.cc{color:var(--on-surface)}`, sem tocar a formatação por célula. **Continua aberto:** a re-tematização própria das células/cabeçalho aos tokens do modelo e a aposentadoria do CSS `.lg-*`/`tokens.css` (esta última está como tarefa de execução T6 do chat). Export passou a refletir a cor do chip do status ([[E-2026-06-25 Tela do projeto no shell DS v2, overhaul do dashboard, De-Para e fixes dark+export]]).
